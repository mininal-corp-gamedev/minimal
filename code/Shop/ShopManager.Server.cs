...
{
	private static void RpcRequestPurchaseServer( string shopId )
	{
	...
	private static void RemoveShopObjectsOnJobChangeServer( Player player )
	{
		if ( !Networking.IsHost )
			return;
		if ( !player.IsValid() )
	...
	private static bool CanBuyByJob( Player buyer, ShopDefinition shop )
	{
		if ( shop is null )
			return false;
