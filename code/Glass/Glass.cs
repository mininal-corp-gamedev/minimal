...
public sealed class Glass : Component, Component.ExecuteInEditor, Component.IDamageable
{
	...
	private Transform FittedWorldTransform;
	...
	protected override void DrawGizmos()
	{
		...
		{
			Gizmo.Draw.LineSphere( PanelToComponentLocal( point, 0 ), 4 );
		}
		...
		foreach ( var shard in Shards.Values )
		{
			...
			{
				var p = Transform.World.PointToLocal( body.Transform.PointToWorld( PanelToComponentLocal( point, 0 ) ) );
	...
	protected override void OnValidate()
	{
		...
		AutoFitFromObjectBounds();

		if ( AutoGeneratePoints || Points is null || Points.Count < 3 )
		{
	...
	private void CreatePrimaryShard()
	{
		...
		CreateShard( GetGlassWorldTransform(), points );
	}

	private void AutoFitFromObjectBounds()
	{
		if ( !AutoFitToObjectBounds )
			return;

		if ( !TryGetFitSource( out var sourceTransform, out var bounds, out var sourceComponents ) )
			return;

		var size = bounds.Maxs - bounds.Mins;

		var validAxisCount = 0;
		if ( size.x > 0.01f ) validAxisCount++;
		if ( size.y > 0.01f ) validAxisCount++;
		if ( size.z > 0.01f ) validAxisCount++;

		if ( validAxisCount < 2 )
			return;

		FittedWorldTransform = sourceTransform;
		HasFittedWorldTransform = true;
		PanelLocalOrigin = (bounds.Mins + bounds.Maxs) * 0.5f;

		if ( AutoDetectPanelPlane )
		{
			if ( size.x <= size.y && size.x <= size.z )
			{
				PanelPlane = GlassPanelPlane.YZ;
			}
			else if ( size.y <= size.x && size.y <= size.z )
			{
				PanelPlane = GlassPanelPlane.XZ;
			}
			else
			{
				PanelPlane = GlassPanelPlane.XY;
			}
		}

		PanelSize = PanelPlane switch
		{
			GlassPanelPlane.XZ => new Vector2( size.x, size.z ),
			GlassPanelPlane.YZ => new Vector2( size.y, size.z ),
			_ => new Vector2( size.x, size.y )
		};

		if ( HideSourceComponents )
		{
	...
	private bool TryGetFitSource( out Transform sourceTransform, out BBox bounds, out List<Component> sourceComponents )
	{
		sourceTransform = Transform.World;
		bounds = default;
		sourceComponents = new List<Component>();

		if ( SourceRenderer.IsValid() )
		{
			sourceTransform = SourceRenderer.GameObject.WorldTransform;
			bounds = SourceRenderer.Model?.Bounds ?? SourceRenderer.GameObject.GetLocalBounds();
			sourceComponents.Add( SourceRenderer );
			return true;
		}

		var renderers = GameObject.Components
			.GetAll<ModelRenderer>( FindMode.EverythingInSelfAndDescendants )
			.Where( x => x.IsValid() && x.Enabled );

		if ( !IncludeDescendantBounds )
		{
			renderers = renderers.Where( x => x.GameObject == GameObject );
		}

		var renderer = renderers.FirstOrDefault();
		if ( renderer.IsValid() )
		{
			sourceTransform = renderer.GameObject.WorldTransform;
			bounds = renderer.Model?.Bounds ?? renderer.GameObject.GetLocalBounds();
			sourceComponents.Add( renderer );
			return true;
		}

		var components = GameObject.Components
			.GetAll<Component>( IncludeDescendantBounds ? FindMode.EverythingInSelfAndDescendants : FindMode.EverythingInSelf )
			.Where( x => x.IsValid() && x.Enabled && x != this )
			.ToList();

		foreach ( var component in components )
		{
			if ( component is not Component.IHasBounds boundsComponent )
				continue;

			sourceTransform = component.GameObject.WorldTransform;
			bounds = boundsComponent.LocalBounds;
	...
	private Transform GetGlassWorldTransform()
	{
		if ( HasFittedWorldTransform )
			return FittedWorldTransform;
		...
		return renderer.GameObject.WorldTransform;
	}

	private void GenerateRectanglePoints()
	{
		var halfWidth = MathF.Max( PanelSize.x, 4.0f ) * 0.5f;
		var halfHeight = MathF.Max( PanelSize.y, 4.0f ) * 0.5f;
	...
	private Vector3 PanelToComponentLocal( Vector2 point, float normalOffset )
	{
		return PanelPlane switch
		{
			GlassPanelPlane.XZ => PanelLocalOrigin + new Vector3( point.x, normalOffset, point.y ),
			GlassPanelPlane.YZ => PanelLocalOrigin + new Vector3( normalOffset, point.x, point.y ),
			_ => PanelLocalOrigin + new Vector3( point.x, point.y, normalOffset )
		};
	}

	private Vector2 ComponentLocalToPanel( Vector3 point )
	{
		var local = point - PanelLocalOrigin;

		return PanelPlane switch
		{
			GlassPanelPlane.XZ => new Vector2( local.x, local.z ),
			GlassPanelPlane.YZ => new Vector2( local.y, local.z ),
	...
	protected override void OnPreRender()
	{
		foreach ( var kv in Shards )
		{
			...
			if ( !shard.IsLoose )
			{
				body.Transform = GetGlassWorldTransform();
			}
	...
	private Shard CreateShard( Transform transform, List<Vector2> points )
	{
		...
		foreach ( var point in points )
		{
			hull.Add( PanelToComponentLocal( point, halfThickness ) );
			hull.Add( PanelToComponentLocal( point, -halfThickness ) );
		}
	...
	[Rpc.Broadcast]
	private void RpcShatterAt( Vector3 worldPosition )
	{
		ShatterAtWorldPosition( worldPosition, true );
	}

	private void ShatterAtWorldPosition( Vector3 worldPosition, bool playSounds )
	{
		if ( !TryFindShardAtWorldPosition( worldPosition, out var shard, out var position ) )
			return;

		if ( playSounds )
		{
			PlayImpactSound( worldPosition );
		}

		ShatterLocalSpace( shard, position, 0, playSounds );
	}

	private bool TryFindShardAtWorldPosition( Vector3 worldPosition, out Shard shard, out Vector2 position )
	{
		foreach ( var candidate in Shards.Values )
		{
			...
			var localPosition = ComponentLocalToPanel( body.Transform.PointToLocal( worldPosition ) );
			if ( !candidate.IsPointInside( localPosition ) )
				continue;
	...
	private void ShatterLocalSpace( Shard shard, Vector2 position, Vector3 impulse, bool playSounds )
	{
		if ( !shard.IsValid() )
			return;
		...
		var area = shard.Area;
		var soundPosition = transform.PointToWorld( PanelToComponentLocal( position, 0 ) );
	...
	private void PlayImpactSound( Vector3 position )
	{
		...
		if ( NextImpactSoundAllowed > 0 )
			return;

		Sound.Play( ImpactSound, position );
		NextImpactSoundAllowed = MathF.Max( ImpactSoundCooldown, 0.0f );
	}

	private void PlayShatterSound( Vector3 position )
	{
		if ( !ShatterSound.IsValid() )
			return;

		Sound.Play( ShatterSound, position );
	}
	...
	public Model CreateModel( List<Vector2> points )
	{
		...
		renderData.Vertices.Add( PanelToComponentLocal( average, halfThickness ) );

		for ( var i = 0; i < renderData.FaceVertexCount - 1; i++ )
		{
			renderData.Vertices.Add( PanelToComponentLocal( points[i], halfThickness ) );
		}
		...
		for ( var i = 0; i < renderData.EdgeQuadCount; i++ )
		{
			...
			renderData.Vertices.Add( PanelToComponentLocal( points[i], -halfThickness ) );
			renderData.Vertices.Add( PanelToComponentLocal( points[next], -halfThickness ) );
			renderData.Vertices.Add( PanelToComponentLocal( points[next], halfThickness ) );
			renderData.Vertices.Add( PanelToComponentLocal( points[i], halfThickness ) );
		}
	...
	private Mesh CreateMesh( RenderData renderData )
	{
		...
		for ( var i = 0; i < renderData.TotalShardVertices; i++ )
		{
			...
			var panelPos = ComponentLocalToPanel( renderData.Vertices[i] );
			var vertexPos = new Vector3( panelPos.x, panelPos.y, 0 );
